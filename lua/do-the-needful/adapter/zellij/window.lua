...
local function compose_job(cmd, cwd)
  Log.trace(sf("zellij.window._compose_job(): cmd %s, cwd %s", cmd, cwd))
  local job_command = table.remove(cmd, 1)
  if not job_command then
    Log.error(sf("zellij.window._compose_job(): no job_command found in cmd %s", cmd))
    return nil
  end
  local job_args = cmd
  local job_tbl = {
  ...
  Log.trace(sf("zellij.window._compose_job(): return job_tbl %s", job_tbl))
  return job_tbl
end
...
local function build_task(task, pane, command)
  ...
  if not cmd then
    Log.error(sf(
      [[zellij.window.build_task(): no return value from %s(). 
        task: %s
        run_adapter: %s
        ]],
  ...
  Log.debug(sf("zellij.window.build_task(): cmd %s", cmd))
  return cmd
end
...
local function build_commands(task, pane)
  ...
  if not command then
    Log.warn(sf(
      [[zellij.window.build_commands(): no command found for run_adapter %s.
        task: %s
        ]],
      const.run_adapter,
      task
...
function M.open(task)
  ...
  Log.trace(sf("zellij.window.open(): cmd %s", cmd))
  if not cmd then
    Log.error(
      [[zellij.window.open(): no return value from build_commands().
    task: %s]],
      task
    )
    return
  ...
  if not task.zellij.window.close then
    if not pane or not pane[1] then
      Log.error(sf(
        [[zellij.window.open(): pane not found when running job: 
      task: %s
      pane: %s
      ]],
        task,
    ...
    pane = pane[1]
    Log.trace(sf(
      [[zellij.window.open(): sending selected task to pane.
    task: %s
    pane: %s
    ]],
      task,
    ...
    if not pane_cmd then
      Log.error(
        "zellij.window.run_tasks(): no return value from build_send_to_pane(). selection %s",
        task
      )
      return
    end
