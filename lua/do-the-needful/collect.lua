...
local function add_metadata(tasks, task_config, source)
  if not task_config or not task_config.tasks then
    dtn.Log.warn(sf("collect._add_metadata(): no tasks found in config %s", task_config))
    return
  end
  for _, task in pairs(task_config.tasks) do
    task.source = source
    dtn.Log.trace(sf("collect._add_metadata(): adding source '%s' to task %s", source, task))
...
function M.tasks()
  ...
  dtn.Log.trace(sf("collect.configs(): parsing configs: %s", sources))
  for _, source in pairs(get_opts().config_order) do
    local path = sources[source].path
    if path then
      local from_json = utils.json_from_path(path)
      if from_json then
        dtn.Log.trace(sf("collect.configs(): composing task: %s from path %s", from_json, path))
  ...
  dtn.Log.debug(sf("collect.configs(): collection %s", tasks))
  return tasks
end
